...
server_Qrev <- function(df,quanti_vars,ID){
  ...
  # MAJ input inclusion -----
    ...
    shiny::updateCheckboxGroupInput(
      session,
      inputId = "incl",
      choices = unique(df[[input$grp]])|>na.omit(),
      selected = unique(df[[input$grp]])|>na.omit()
    )
  })
  
  # MAJ bornes à 0 lors du changement de variables -------
  ...
  df_filtre <- shiny::eventReactive(input$go, {
    ...
    if (grp_go() != "Aucune") {
      ...
      # Retrait des niveaux inutilisés en conservant labels
  ...
  output$date_removed <- shiny::renderText({
    ...
    if (ncol(df_date) > 0) {
        "Les variables dates ne sont pas prises en charge par RastaRocket et ont été retirées du tableau."
    } else {
      ""
    }
  ...
  # Description de la variable numérique à investiguer
  ...
  # Expression de la condition de repérage d'outliers
  ...
  # Pour maj bouton téléchargement
